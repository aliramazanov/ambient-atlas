...
export interface InspectResult {
	zones: { name: string; tier: Tier; category: string; distDeg: number; rd: number }[];
...
/** Everything affecting a clicked point: overlapping zones, nearest cities,
 * and the (nearest-centroid) country's metrics. */
export function inspectLocation(lat: number, lng: number): InspectResult {
	...
	// UV is a latitude gradient rather than a circle.
	if (Math.abs(lat) <= 35) {
		zoneHits.push({
	...
	const nearCities = cities
		.map((c) => ({ name: c.name, d: angularDistanceDeg(lat, lng, c.lat, c.lng) }))
		.sort((a, b) => a.d - b.d)
		.slice(0, 3)
	...
	if (best) {
		...
		for (const m of METRICS) {
			...
			if (v != null) metrics.push({ label: m.label, value: m.format(v), unit: m.unit });
		}
		country = { name: best.name, iso3: best.iso3, metrics };
	}

	return {
		zones: zoneHits.map(({ name, tier, category, distDeg, rd }) => ({ name, tier, category, distDeg, rd })),
		cities: nearCities,
		country
	};
}
