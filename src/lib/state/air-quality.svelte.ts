...
/** Live US AQI for the city list, fetched on demand from the Open-Meteo Air Quality API.
 * `aqi` is one big parallel array (indexed by city) that we only ever replace wholesale, so it is $state.raw
 * to skip deep-proxy overhead: it is read per-frame in the render loop where proxy get-traps would add up. */
